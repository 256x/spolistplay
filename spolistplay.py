...
def init_iceberg_colors():
    """Initialize curses color pairs for an iceberg-like theme with transparent background."""
    if not curses.has_colors():
        logging.warning("Terminal does not support colors.")
        return

    default_bg = -1  # Use terminal's default background
    try:
        # Color pair 1: White text, transparent background
        ...
        # Color pair 2: Bright cyan for selection highlight, transparent background
        if curses.COLORS >= 16:
            curses.init_pair(2, 110, default_bg)  # Selection highlight
        else:
            curses.init_pair(2, curses.COLOR_CYAN, default_bg)
        curses.init_pair(3, 109, default_bg)  # Status text
        curses.init_pair(4, 146, default_bg)  # Progress bar
        curses.init_pair(5, 216, default_bg)  # Headers
        curses.init_pair(6, 167, default_bg)  # Errors
        curses.init_pair(7, 150, default_bg)  # Playing status
        logging.debug("Initialized color pairs with default background.")
    except curses.error as e:
        logging.warning(f"Failed to initialize color pairs with default background ({default_bg}): {e}. Trying COLOR_BLACK (0).")
        ...
        try:
            ...
            logging.debug(f"Initialized color pairs with COLOR_BLACK background ({default_bg}).")
        except curses.error as e_black:
            logging.error(f"Failed to initialize color pairs with COLOR_BLACK background ({default_bg}): {e_black}. Colors may not display correctly.")

def init_standard_colors():
    """Initialize curses color pairs for 8-color terminals with transparent background."""
    if not curses.has_colors():
        return
    ...
    try:
        curses.init_pair(1, curses.COLOR_WHITE, default_bg)
        curses.init_pair(2, curses.COLOR_CYAN, default_bg)  # Selection highlight
        ...
        logging.warning(f"Failed to initialize standard color pairs with default background ({default_bg}): {e}. Trying COLOR_BLACK (0).")
        default_bg = curses.COLOR_BLACK
        try:
            curses.init_pair(1, curses.COLOR_WHITE, default_bg)
            curses.init_pair(2, curses.COLOR_CYAN, default_bg)
            ...
            logging.error(f"Failed to initialize standard color pairs with COLOR_BLACK background ({default_bg}): {e_black}. Colors may not display correctly.")
...
def select_playlist_curses(stdscr, playlists):
    ...
    while True:
        ...
        stdscr.addstr(0, 0, header[:max_x], curses.color_pair(3))
        ...
        for idx in range(start, end):
            ...
            if row < max_y - 1:
                if idx == selected:
                    stdscr.addstr(row, 0, line[:max_x], curses.color_pair(2) | curses.A_REVERSE)
                else:
                    stdscr.addstr(row, 0, line[:max_x], curses.color_pair(1))

        # Command help at bottom
        help_text = "[ move: hjkl/arrows | select: Enter | cancel: ESC/Q ]"
        stdscr.addstr(max_y - 1, 0, help_text[:max_x], curses.color_pair(3))
...
def select_device_curses(stdscr):
    ...
    try:
        ...
        stdscr.erase()
        stdscr.addstr(1, 0, "Failed to fetch devices.", curses.color_pair(3))
        stdscr.addstr(3, 0, str(e), curses.color_pair(1))
    ...
    if not devices:
        stdscr.erase()
        stdscr.addstr(1, 0, "No active Spotify devices found.", curses.color_pair(3))
    ...
    while True:
        ...
        # Header
        stdscr.addstr(0, 0, "- Device", curses.color_pair(3))
            ...
            if needs_redraw:
                ...
                if display_track_info:
                    ...
                    item_display_data.extend([
                        ...
                        ("Progress:", progress_time, 6, 7),
                    ])
                    status_row_in_body = 8
                else:
                    waiting_message_start_col_in_body = 0
                    body_win.addstr(2, waiting_message_start_col_in_body, truncate_text("Waiting for playback info...", usable_x - waiting_message_start_col_in_body), curses.color_pair(3))
                    if cached_playback_state is None:
                        body_win.addstr(3, waiting_message_start_col_in_body, truncate_text("Attempting to fetch playback state...", usable_x - waiting_message_start_col_in_body), curses.color_pair(3))
                        body_win.addstr(4, waiting_message_start_col_in_body, truncate_text("Ensure a device is active and playing.", usable_x - waiting_message_start_col_in_body), curses.color_pair(3))
                    else:
                        body_win.addstr(3, waiting_message_start_col_in_body, truncate_text("Playback stopped or no track loaded.", usable_x - waiting_message_start_col_in_body), curses.color_pair(3))
                        current_active_device_name = display_playback_state.get("device", {}).get("name")
                        if current_active_device_name:
                            body_win.addstr(4, waiting_message_start_col_in_body, truncate_text(f"Active Device: {current_active_device_name}", usable_x - waiting_message_start_col_in_body), curses.color_pair(3))
                ...
                if separator_row_in_body < body_h:
                    ...
                    body_win.addstr(separator_row_in_body, 0, progress_bar, curses.color_pair(4))
